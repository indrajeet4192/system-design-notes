...
- One machine can only hold so much data. The lecture put it bluntly: 100 GB fits fine in 200 GB of storage, but 1 TB — 100 TB — the disk fills up. You cannot expand a single box vertically forever.
...
**Why we bother** (the lecture gave three reasons, all raising read throughput in the end):
...
You don't just turn it on and hope — the lecture gave a 3-step bootstrap, and replicas *can* be bootstrapped from another follower:
...
The lecture offered **three resolution strategies**, and interviewers love this list:
...
> Real products run this way (the transcript garbled the names; it meant) **Amazon DynamoDB, Riak, Cassandra** — the no-leader family. Worth naming in an interview.
...
Replication hit a wall and here's where it fails (the lecture's two exact complaints):
...
So instead of copying everything everywhere, we **divide the data by logic**: put one part on node A, the other part on node B — and then (a preview of the final section) replicate those *smaller* partitions. The lecture gave both of the rules of the game, in order:
...
Small vocabulary guardrail: the lecture kept saying we "cannot expand vertically forever" — that's the *scale-up* path of just buying a bigger single machine (more RAM/storage/CPU), which always ends at "the disk fills up." Partitioning is the **horizontal** path — more machines, each holding a slice of the rows. When people say **sharding**, they mean this horizontal splitting; that's what both strategies below are doing.
...
Cut the dataset by the value of a key. Stock example from the lecture: a user table with 100,000 IDs → split 50,000 / 50,000. User **1–50,000 → partition 1**, user **50,001–100,000 → partition 2**. Trivial to retrieve: you know instantly which node owns a given user.
...
And the fix is never free: re-sharding means redistributing the data, reconceiving the boundaries, replacing nodes — the lecture listed it as "redistribute data, reconfigure, replace the node." So we don't wait for the hotspot to burn the node; we watch the request timestamps and act before the imbalance turns into an outage.
...
- **Hot spots / skew** — the partition-level version of imbalance; one shard fries at ~10,000 req/s while its sibling sleeps at ~100. The lecture's exact word is **hotspot**.
...
Either strategy, the lesson is the same: partition boundaries make writes neat and local, but every read that crosses a boundary pays a price. The shard key is what decides how often a query stays inside one partition.
